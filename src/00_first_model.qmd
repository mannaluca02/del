...
Sowohl das Train als auch das Test Dataframe hat eine ausgewogene Klassenverteilung. Es benötigt keine Oversampling oder Undersampling einer Klasse.
...
- Die Klassen `buildings` und `street` könnten Problematisch zu unterscheiden sein. Bei dem ersten Beispielbild von `street` sieht man auch Gebäude. Dies kann schwer zu unterscheiden sein kann für das Modell.
...
Es werden immer die Werte von dem Baseline Modell verwendet für die Vergleiche. Ausser es wird explizit genannt, dass die Werte von dem 5-fachen-CV verwendet wird. 
...
Geplant war ein Vergleich der Gewichts-Initialisierung (Kaiming vs. Xavier vs. PyTorch Default). PyTorch nutzt bei `nn.Conv2d` und `nn.Linear` jedoch bereits Kaiming Uniform als Default, sodass kein messbarer Effekt gegenüber Xavier zu erwarten war. 
...
|---|---|---|---|---|
| Baseline | SGD, kein BN, keine Regularisierung | 84.89% | — | — |
| H1 | Modelltiefe (4 Conv-Blöcke) | 85.70% | +0.81 pp | ✓ |
| H2 | Modellbreite (2× Filter) | 86.32% | +1.43 pp | ✓ (4× im Sweep) |
| H3 | Kernel Size 5×5 | 84.53% | −0.36 pp | ✗ |
| H4 | AvgPool statt MaxPool | 83.18% | −1.71 pp | ✗ |
| H5 | Dropout (p=0.5) | 85.92% | +1.03 pp | ✓ |
| H6 | Stärkere Augmentation | 85.43% | +0.54 pp | ✓ |
| H7 | Weight Decay (5e-4) | 85.96% | +1.07 pp | ✓ |
| H8 | BatchNorm (bs=16) | 87.60% | +2.71 pp | ✓ |
| H10 | Adam Optimizer (lr=5e-4) | 87.39% | +2.50 pp | ✓ |
| H11 | Transfer Learning Finetune | ~91.00% | ~+6.11 pp | ✗ (Regelwerk) |
| **Final** | **Kombination H1+H2+H5+H6+H7+H8+H10** | **89.74% val / 89.20% test** | **+4.85 pp** | — |

## Erkenntnisse nach Kategorie

**Modellkomplexität**
* Tiefe (H1) bringt +0.81 pp ohne Mehrkosten durch Overfitting, weil das zusätzliche MaxPool nach Block 4 den FC-Eingang verkleinert und die Parameterzahl von 549k auf 360k senkt.
* Breite (H2) liefert als isoliertes Experiment +1.43 pp, jedoch mit grösserem Generalization Gap und instabilem Training in frühen Epochen. Ohne Regularisierung fliesst die Zusatzkapazität in Overfitting. Der Sweep bestätigt base_filters=64 als optimalen Sweet Spot, was einer 4× Verbreiterung gegenüber der Baseline entspricht. Breite zahlt sich also erst in Kombination mit Regularisierung aus.

**Conv Layer Settings**
* Kernel Size 5×5 (H3) bringt −0.36 pp. Bei kleinen Eingabebildern (64×64) und flachen Architekturen überwiegt der Mehraufwand den Kontextgewinn nicht. 3×3 bleibt die robustere Wahl.
* AvgPool (H4) liegt −1.71 pp unter der Baseline und zeigt ein anderes Klassenprofil: bessere Buildings Erkennung, schwächere Mountain und Forest Erkennung. MaxPool wird im Finalmodell beibehalten, weil es bei der Zielmetrik Overall Accuracy überlegen ist.

**Regularisierung**
* Dropout (H5), Augmentation (H6) und Weight Decay (H7) wirken auf unterschiedlichen Ebenen: Dropout deaktiviert Neuronen zufällig, Augmentation variiert den Input, Weight Decay begrenzt die Gewichtsgrösse direkt. Alle drei verbessern die Val Accuracy um +0.54 bis +1.07 pp und reduzieren den Generalization Gap von ~0.20 auf ~0.05.
* Ein konsistentes Muster: Die Klasse Buildings profitiert von jeder Regularisierungsmassnahme am stärksten. Sie scheint besonders anfällig für Overfitting zu sein, was auf heterogene Erscheinungsbilder (Fassaden unter verschiedenen Beleuchtungen, Winkeln) hindeutet.
* Die drei Mechanismen sind komplementär und werden im Finalmodell kombiniert.

**BatchNorm**
* H8 ist der stärkste Einzelbaustein unter den eigenständig trainierten Methoden (+2.71 pp, 87.60%). Im Gegensatz zu reinen Regularisierern verbessert BatchNorm gleichzeitig Optimierung (tieferer Train Loss, schnellere Konvergenz) und Generalisierung (kleinerer Gap). Wichtig: BatchNorm erfordert eine Batch Size Anpassung. bs=64 führte zu Trainingskollaps, bs=16 ist im isolierten Experiment optimal, bs=32 im vollständigen Finalmodell (durch den Sweep bestätigt).

**Adam Optimizer**
* Adam (H10, lr=5e-4) erreicht mit +2.50 pp (87.39%) denselben Bereich wie BatchNorm auf einem komplett anderen Weg: Adam passt die Lernrate pro Parameter adaptiv an, statt Aktivierungen zu normalisieren. Die optimale Lernrate liegt um Faktor 100 unter SGD (5e-4 vs. 0.05). Beide Techniken sind komplementär und ihre Kombination im Finalmodell liefert mehr als die Summe der Einzelteile.

**Transfer Learning**
* Finetune (H11) ist mit ~91.0% die stärkste Einzelmassnahme (+6.11 pp), aber per Regelwerk für das Finalmodell ausgeschlossen. Der Mechanismus unterscheidet sich grundlegend: statt Optimierung einer zufälligen Architektur startet Finetune mit Features, die auf 1.4M ImageNet-Bildern vortrainiert wurden. Der hohe Generalization Gap (~0.37 Loss) zeigt, dass ResNet18 für ~14k Trainingsbilder überdimensioniert ist und ohne zusätzliche Regularisierung überanpasst.
* Feature Extract (~79%) liegt deutlich unter der Baseline. Die Domain Lücke zwischen ImageNet und Intel Image Classification ist zu gross, um ohne Backbone Anpassung überbrückt zu werden.

## Kombinationseffekt

Das Finalmodell kombiniert die sieben wirksamsten eigenständig trainierten Techniken: 4 Conv Blöcke (H1), Breite 4× (H2 via Sweep), Dropout (H5), Augmentation (H6), Weight Decay (H7), BatchNorm (H8) und Adam (H10). Das Ergebnis (Val Acc 89.74%, Test Acc 89.20%) liegt mit +4.85 pp über der Baseline und übertrifft jede Einzelmassnahme ausser Transfer Learning. Die Kombination wirkt stärker als die Summe der Einzelteile, weil sich die Mechanismen gegenseitig verstärken: BatchNorm stabilisiert den Gradientenfluss, Adam nutzt diese Stabilität für schnellere Konvergenz, Dropout und Augmentation verhindern Overfitting bei der gestiegenen Modellkapazität.

---
## Literatur

- Bayle, P., Bayle, A., Janson, L. & Mackey, L. (2020). *Cross-validation Confidence Intervals for Test Error*. Advances in Neural Information Processing Systems 33 (NeurIPS 2020). https://arxiv.org/abs/2007.12671
